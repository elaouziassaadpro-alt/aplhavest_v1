<div>
    ...
    <form wire:submit.prevent="update">
        @forelse($personnes as $index => $ph)
            <div class="border rounded p-3 mb-4">
                {{-- LIGNE 1 --}}
                <div class="row mb-2">
                    <div class="col-md-3 mb-2">
                        <label>Nom / Raison sociale</label>
                        <input type="text" class="form-control" wire:model="personnes.{{ $index }}.nom_rs" @disabled(!$editing)>
                    </div>
                    <div class="col-md-3 mb-2">
                        <label>Prénom</label>
                        <input type="text" class="form-control" wire:model="personnes.{{ $index }}.prenom" @disabled(!$editing)>
                    </div>
                    <div class="col-md-2 mb-2">
                        <label>CIN / Passeport</label>
                        <input type="text" class="form-control" wire:model="personnes.{{ $index }}.identite" @disabled(!$editing)>
                    </div>
                    <div class="col-md-2 mb-2">
                        <label>Nationalité</label>
                        <x-searchable-select :options="$pays" wireModel="personnes.{{ $index }}.nationalite_id" :selected="$ph['nationalite_id'] ?? ''" :disabled="!$editing" />
                    </div>
                    @if($editing)
                    <div class="col-md-1 mt-4">
                        <button type="button" class="btn btn-danger btn-sm" wire:click="removePersonne({{ $index }})">Supprimer</button>
                    </div>
                    @endif
                </div>
                ...
                <div class="row mb-2">
                    <div class="col-md-3 mb-2">
                        ...
                        <input type="text" class="form-control" wire:model="personnes.{{ $index }}.fonction" @disabled(!$editing)>
                    </div>
                </div>
                    ...
                    <div class="col-md-6 mb-2">
                        @if($editing)
                            <label class="btn btn-primary w-100">
                                ...
                                <input type="file" wire:model="personnes.{{ $index }}.new_cin_file" class="d-none">
                            </label>
                        @endif
                        @if(!empty($ph['existing_cin_file']))
                            <a href="{{ asset('storage/' . $ph['existing_cin_file']) }}" target="_blank" class="btn btn-sm btn-outline-primary mt-1 w-100" download>
                    ...
                    <div class="col-md-6 mb-2">
                        @if($editing)
                            <label class="btn btn-warning w-100 text-white">
                                <i class="ti ti-upload"></i> Fichier d'habilitation
                                <input type="file" wire:model="personnes.{{ $index }}.new_habilitation_file" class="d-none">
                            </label>
                        @endif
                        @if(!empty($ph['existing_habilitation_file']))
                            <a href="{{ asset('storage/' . $ph['existing_habilitation_file']) }}" target="_blank" class="btn btn-sm btn-outline-primary mt-1 w-100" download>
                                <i class="ti ti-download"></i> Télécharger
                            </a>
                        @endif
                    </div>
                ...
                <div class="row mb-2 align-items-center">
                    <div class="col-md-1 mb-2">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" wire:model.live="personnes.{{ $index }}.ppe" @disabled(!$editing)>
                        </div>
                        <label>{{ $ph['ppe'] ? 'Oui' : 'Non' }}</label>
                    </div>

                    @if($ph['ppe'])
                    <div class="col-md-3 mb-2">
                        <label>Libellé PPE</label>
                        <x-searchable-select :options="$ppes" wireModel="personnes.{{ $index }}.libelle_ppe" :selected="$ph['libelle_ppe'] ?? ''" :disabled="!$editing" />
                    </div>
                    @endif
                    ...
                    <div class="col-md-1 mb-2">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" wire:model.live="personnes.{{ $index }}.lien_ppe" @disabled(!$editing)>
                        </div>
                        <label>{{ $ph['lien_ppe'] ? 'Oui' : 'Non' }}</label>
                    </div>

                    @if($ph['lien_ppe'])
                    <div class="col-md-3 mb-2">
                        <label>Type de lien PPE</label>
                        <x-searchable-select :options="$ppes" wireModel="personnes.{{ $index }}.libelle_ppe_lien" :selected="$ph['libelle_ppe_lien'] ?? ''" :disabled="!$editing" />
                    </div>
                    @endif
                </div>
            </div>
        @empty
            <h5 class="text-center">Aucune personne habilitée enregistrée</h5>
        @endforelse

        <div class="text-start">
            <a href="{{ route('personneshabilites.create', ['etablissement_id' => $etablissement_id, 'redirect_to' => 'dashboard']) }}" class="btn btn-light-info btn-sm mb-2">
                Ajouter une personne
            </a>
        </div>

        @if($editing && count($personnes) > 0)
