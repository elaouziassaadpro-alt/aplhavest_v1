<div>
    ...
    <form wire:submit.prevent="update">
        @forelse($administrateurs as $index => $admin)
            <div class="border rounded p-3 mb-4">
                {{-- LIGNE 1 --}}
                <div class="row mb-2">
                    <div class="col-md-3 mb-2">
                        <label>Nom</label>
                        <input type="text" class="form-control" wire:model="administrateurs.{{ $index }}.nom" @disabled(!$editing)>
                    </div>
                    <div class="col-md-3 mb-2">
                        <label>Prénom</label>
                        <input type="text" class="form-control" wire:model="administrateurs.{{ $index }}.prenom" @disabled(!$editing)>
                    </div>
                    <div class="col-md-2 mb-2">
                        <label>Date de naissance</label>
                        <input type="date" class="form-control" wire:model="administrateurs.{{ $index }}.date_naissance" @disabled(!$editing)>
                    </div>
                    <div class="col-md-2 mb-2">
                        <label>CIN / Passeport</label>
                        <input type="text" class="form-control" wire:model="administrateurs.{{ $index }}.identite" @disabled(!$editing)>
                    </div>
                    @if($editing)
                    <div class="col-md-1 mt-4">
                        <button type="button" class="btn btn-danger btn-sm" wire:click="removeAdministrateur({{ $index }})">Supprimer</button>
                    </div>
                    @endif
                </div>
                ...
                <div class="row mb-2">
                    <div class="col-md-3 mb-2">
                        ...
                        <x-searchable-select :options="$pays" wireModel="administrateurs.{{ $index }}.nationalite_id" :selected="$admin['nationalite_id'] ?? ''" :disabled="!$editing" />
                    </div>
                    <div class="col-md-3 mb-2">
                        <label>Fonction</label>
                        <input type="text" class="form-control" wire:model="administrateurs.{{ $index }}.fonction" @disabled(!$editing)>
                    </div>
                </div>
                ...
                <div class="row mb-2">
                    <div class="col-md-6 mb-2">
                        @if($editing)
                            <label class="btn btn-primary w-100">
                                ...
                                <input type="file" wire:model="administrateurs.{{ $index }}.new_cin_file" class="d-none">
                            </label>
                        @endif
                        @if(!empty($admin['existing_cin_file']))
                            <a href="{{ asset('storage/' . $admin['existing_cin_file']) }}" target="_blank" class="btn btn-sm btn-outline-primary mt-1 w-100" download>
                    ...
                    <div class="col-md-6 mb-2">
                        @if($editing)
                            <label class="btn btn-warning w-100 text-white">
                                <i class="ti ti-upload"></i> PV de nomination
                                <input type="file" wire:model="administrateurs.{{ $index }}.new_pvn_file" class="d-none">
                            </label>
                        @endif
                        @if(!empty($admin['existing_pvn_file']))
                            <a href="{{ asset('storage/' . $admin['existing_pvn_file']) }}" target="_blank" class="btn btn-sm btn-outline-primary mt-1 w-100" download>
                                <i class="ti ti-download"></i> Télécharger
                            </a>
                        @endif
                    </div>
                ...
                <div class="row mb-2 align-items-center">
                    <div class="col-md-1 mb-2">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" wire:model.live="administrateurs.{{ $index }}.ppe" @disabled(!$editing)>
                        </div>
                        <label>{{ $admin['ppe'] ? 'Oui' : 'Non' }}</label>
                    </div>

                    @if($admin['ppe'])
                    <div class="col-md-3 mb-2">
                        <label>Libellé PPE</label>
                        <x-searchable-select :options="$ppes" wireModel="administrateurs.{{ $index }}.ppe_id" :selected="$admin['ppe_id'] ?? ''" :disabled="!$editing" />
                    </div>
                    @endif
                    ...
                    <div class="col-md-1 mb-2">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" wire:model.live="administrateurs.{{ $index }}.lien_ppe" @disabled(!$editing)>
                        </div>
                        <label>{{ $admin['lien_ppe'] ? 'Oui' : 'Non' }}</label>
                    </div>

                    @if($admin['lien_ppe'])
                    <div class="col-md-3 mb-2">
                        <label>Type de lien PPE</label>
                        <x-searchable-select :options="$ppes" wireModel="administrateurs.{{ $index }}.lien_ppe_id" :selected="$admin['lien_ppe_id'] ?? ''" :disabled="!$editing" />
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="text-start">
            <a href="{{ route('administrateurs.create', ['etablissement_id' => $etablissement_id, 'redirect_to' => 'dashboard']) }}" class="btn btn-light-info btn-sm mb-2">
                Ajouter un administrateur
            </a>
        </div>

        @if($editing && count($administrateurs) > 0)
            ...
            <div class="modal-dialog modal-xl modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Analyse du risque - Administrateurs</h5>
