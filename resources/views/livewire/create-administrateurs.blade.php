<div>
    <x-breadcrumb-header 
        ...
        activePage="Administrateur / Dirigeant" 
    />
    <x-notification />
    ...
    <div class="max-w-7xl mx-auto bg-white cardetablissement shadow-sm border border-gray-100 mt-4 ">
        <div class="px-6 py-4 border-b bg-gray-50 rounded-t-xl">
            <h2 class="text-xl font-semibold text-gray-800">Administrateur ou Dirigeant</h2>
        </div>
                ...
                @foreach($rows as $index => $row)
                    <div class="card mb-3 border">
                        <div class="card-header bg-light d-flex justify-content-between align-items-center">
                            <span class="fw-bold">Administrateur #{{ $index + 1 }}</span>
                            @if(count($rows) > 1)
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="text" wire:model="rows.{{ $index }}.nom" class="form-control" placeholder="Nom">
                                </div>
                                ...
                                <!-- Pays (Demeure) -->
                                <div class="col-md-3 mb-3">
                                    <label class="form-label">Pays de demeure</label>
                                    <x-searchable-select :options="$pays" wireModel="rows.{{ $index }}.pays_id" :selected="$row['pays_id'] ?? ''" :disabled="false" />
                                </div>
                            ...
                            <div class="row">
                                ...
                                <!-- File Upload PV -->
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">PV de nomination (Fichier)</label>
                                    <input type="file" wire:model="rows.{{ $index }}.pvn_file" class="form-control">
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" wire:model.live="rows.{{ $index }}.is_ppe" id="ppe_admin_{{ $index }}">
                                        <label class="form-check-label" for="ppe_admin_{{ $index }}">Est PPE ?</label>
                                    </div>
                                    @if($row['is_ppe'])
                                ...
                                <div class="col-md-6 mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" wire:model.live="rows.{{ $index }}.is_ppe_lien" id="ppe_lien_admin_{{ $index }}">
                                        <label class="form-check-label" for="ppe_lien_admin_{{ $index }}">A un lien avec PPE ?</label>
                                    </div>
                                    @if($row['is_ppe_lien'])
            ...
            <div class="row mt-3">
                <div class="col-md-3">
                    <button type="button" class="btn btn-light-info" wire:click="addRow">
                        <i class="ti ti-plus me-1"></i> Ajouter un administrateur
                    </button>
                </div>
            </div>
    ...
    <div class="row mt-4 align-items-center">
        ...
        @if($redirect_to !== 'dashboard')
            <div class="col-auto">
                 <a href="{{ route('personneshabilites.create', ['etablissement_id' => $etablissement->id]) }}"
                class="skip-link">
                    Skip
                </a>
            </div>
                                    ...
                                    @foreach($analysisResults as $index => $result)
                                        ...
                                        @endphp
                                         <tr class="{{ $isInterdit ? 'table-danger' : 
                                            ...
                                            <td>{{ $result['data']['nom'] }} {{ $result['data']['prenom'] }}</td>
