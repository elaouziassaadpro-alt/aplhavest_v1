<div>
  <x-notification />

    <x-breadcrumb-header
        title="Liste des Actionnaires"
        activePage="Actionnaires"
    />

    {{-- Search & Actions --}}
    <div class="card card-body boutons_header">
        <div class="row align-items-center">
            <div class="col-md-4 col-xl-3">
                <form class="position-relative">
                    <input type="text" class="form-control product-search ps-5"
                           wire:model.live.debounce.300ms="search"
                           placeholder="Recherche par nom ou prénom...">
                    <i class="ti ti-search position-absolute top-50 start-0 translate-middle-y fs-6 text-dark ms-3"></i>
                </form>
            </div>

            <div class="col-md-8 col-xl-9 text-end d-flex justify-content-md-end justify-content-center mt-3 mt-md-0 gap-2">
                @if(count($selected) > 0)
                    <button wire:click="deleteSelection"
                        wire:confirm="Voulez-vous vraiment supprimer les actionnaires sélectionnés ?"
                        class="btn btn-light-danger text-danger d-flex align-items-center font-medium">
                        <i class="ti ti-trash me-1 fs-5"></i> Supprimer la sélection
                    </button>
                @endif

                <a href="{{ route('actionnariat.create') }}" class="btn btn-info d-flex align-items-center">
                    <i class="ti ti-users text-white me-1 fs-5"></i> Ajouter un actionnaire
                </a>
            </div>
        </div>
    </div>

    {{-- Table --}}
    <div class="card card-body">
        <div class="table-responsive">
            <table class="table search-table align-middle text-nowrap">
                <thead>
                    <tr>
                        <th class="text-center">
                            <input type="checkbox" class="form-check-input" wire:model.live="selectAll">
                        </th>
                        <th>Etablissement</th>
                        <th>Nom / Raison sociale</th>
                        <th>Prénom</th>
                        <th>CIN / Passport</th>
                        <th>% Capital</th>
                        <th>N* de titres</th>
                        <th>Note</th>
                        <th>Risque</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($actionnariats as $actionnaire)
                    @php
                            $actionnaireNiveauRisque = $actionnaire->note;
                            if ($actionnaireNiveauRisque >= 30) {
                                $color = 'text-warning';
                                $risk = 'High Risk';
                            } elseif ($actionnaireNiveauRisque >=7)     {
                                $color = 'text-primary';
                                $risk = 'Medium Risk';
                            } else {
                                $color = 'text-success';
                                $risk = 'Low Risk';
                            }
                        @endphp
                        <tr wire:key="act-{{ $actionnaire->id }}">
                            <td class="text-center">
                                <input type="checkbox" class="form-check-input"
                                       wire:model.live="selected" value="{{ $actionnaire->id }}">
                            </td>
                            <td>{{ $actionnaire->etablissement->name }}</td>
                            <td>{{ $actionnaire->nom_rs }}</td>
                            <td>{{ $actionnaire->prenom ?? '—' }}</td>
                            <td>{{ $actionnaire->identite ?? '—' }}</td>
                            <td>{{ $actionnaire->pourcentage_capital !== null ? $actionnaire->pourcentage_capital.' %' : '—' }}</td>
                            <td>{{ $actionnaire->nombre_titres ?? '—' }}</td>
                            <td class="{{ $color }}">{{ $actionnaireNiveauRisque }}</td>
                            <td class="{{ $color }}">{{ $risk }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="text-center text-muted">Aucun actionnaire enregistré</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>
</div>
