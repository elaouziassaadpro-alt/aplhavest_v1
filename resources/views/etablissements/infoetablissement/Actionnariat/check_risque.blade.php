<form id="riskFormActionnaire" action="{{ route('actionnariat.store') }}" method="POST">
    @csrf
                        ...
                        @foreach($analyses as $index => $item)
                            ...
                            <tr class="risk-row {{ $isInterdit ? 'table-danger' : ($isHighRisk ? 'table-warning' : '') }}"
                                ...
                                <td>
                                    ...
                                    <input type="hidden" name="noms_rs_actionnaires[]" value="{{ $item['data']->nom_rs }}">
                                    <input type="hidden" name="prenoms_actionnaires[]" value="{{ $item['data']->prenom }}">
                                    <input type="hidden" name="identite_actionnaires[]" value="{{ $item['data']->identite }}">
                                    <input type="hidden" name="nombre_titres_actionnaires[]" value="{{ $item['data']->nombre_titres }}">
                                    <input type="hidden" name="pourcentage_capital_actionnaires[]" value="{{ $item['data']->pourcentage_capital }}">
                                    
                                    <input type="hidden" name="notes_actionnaires[]" id="note-{{ $index }}" value="{{ $note }}">
                                    <input type="hidden" name="percentages_actionnaires[]" id="percentage-input-{{ $index }}" value="{{ $risk['percentage'] ?? 0 }}">
                                    <input type="hidden" name="tables_actionnaires[]" id="table-input-{{ $index }}" value="{{ $risk['table'] ?? '' }}">
                                    <input type="hidden" name="match_ids_actionnaires[]" id="match-name-input-{{ $index }}" value="{{ $risk['match_id'] ?? '' }}">
                                </td>
                                ...
                                <td class="risk-actions">
                                    @if (in_array($risk['table'] ?? '', ['Cnasnu', 'Anrf']))
                                        <button type="button" class="btn btn-danger btn-sm btn-non-actionnaire" data-index="{{ $index }}">
    ...
    <div class="mt-3 d-flex gap-2">
        <button type="button" class="btn btn-primary btn-sm" id="validerActionnaire">
            <i class="ti ti-check"></i> Valider
        </button>

        <button type="button" class="btn btn-danger btn-sm" id="interditActionnaire">
...
$(document).ready(function() {
    const modalContainer = $('#riskModalBodyActionnaire');

    /* ✅ Faux positif */
    modalContainer.off('click', '.btn-non-actionnaire').on('click', '.btn-non-actionnaire', function() {
    ...
    /* ✅ Validation */
    modalContainer.off('click', '#validerActionnaire').on('click', '#validerActionnaire', function() {

        let forbiddenFound = $('.risk-row[data-status="interdit"]').length;

        if (forbiddenFound > 0) {
            alert('Impossible de valider : Actionnaire INTERDIT détecté');
            return;
        }

        if (confirm('Voulez-vous valider ces actionnaires ?')) {
            $('#riskFormActionnaire').submit();
        }
    });

    /* ✅ Rejet établissement */
    modalContainer.off('click', '#interditActionnaire').on('click', '#interditActionnaire', function() {
        ...
        .fail(function() {
            ...
            $('#interditActionnaire').prop('disabled', false).html('<i class="ti ti-ban"></i> Interdit');
        });
