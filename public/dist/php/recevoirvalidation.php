...
if(isset($_POST['idetablissement']) && isset($_POST['action']))
{
	...
	try {
		...
		if($idToValidate != "" && $_POST['action'] == 'validation')
		{
			...
			echo json_encode(['validation' => true, 'message' => "L'établissement a été validé !", 'titre' => 'Validation effectué', 'icone' => 'ti ti-check fs-10 text-success', 'par' => $par, 'date' => $date, 'heure' => $heure]);
		}
		else if($idToValidate != "" && $_POST['action'] == 'rejet')
		{
			...
			echo json_encode(['rejet' => true, 'message' => "L'établissement a été rejeté !", 'titre' => 'Rejet effectué', 'icone' => 'ti ti-check fs-10 text-success', 'par' => $par, 'date' => $date, 'heure' => $heure]);
		}
