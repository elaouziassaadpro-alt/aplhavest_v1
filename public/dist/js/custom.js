...
document.getElementById('delete-selection').addEventListener('click', function () {
    ...
    const ids = [...document.querySelectorAll('.actionnaire-chkbox:checked')]
        .map(cb => cb.value);

    if (ids.length === 0) return;

    if (!confirm('Confirmer la suppression des actionnaires sélectionnés ?')) return;

    fetch("{{ route('actionnariats.bulkDelete') }}", {
        method: "DELETE",
        headers: {
